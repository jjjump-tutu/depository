...
for i in range(seq):
    ...
    # model.summary()
    feature_map_model = Model(inputs=[model.input],outputs=[model.get_layer("activation_12").output])
    ...
    with open('activation_12.npz', 'wb') as f:
        np.savez(
            f,
            trainX=feature_list,
            trainY=lable_list,
            testX=feature_list_test,
            testY=lable_list_test
        )
